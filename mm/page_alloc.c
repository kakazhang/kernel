						...
						int migratetype)
{
	...
	for (current_order = order; current_order < MAX_ORDER; ++current_order) {
		...
		rmv_page_order(page);
		area->nr_free--;
		//just expand when order is small than current_order
...
static void try_quick_free_memory(void) {
	struct task_struct *p;
	struct task_struct *selected =NULL;
	int min_adj = 2;

	/*
	*iterator all the tasks,
	* kill all process whose oom_adj is large than 2
	*/
...
rebalance:
	if (rebalance_count++ > MAX_TRY_COUNT) {
		pr_err("kill to free memory");
		try_quick_free_memory();
	}
		...
		unsigned long *zones_size, unsigned long *zholes_size)
{
	...
	init_waitqueue_head(&pgdat->congest_wait);
